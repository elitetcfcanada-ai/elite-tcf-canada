...
/**
 * Insere l'epreuve Expression orale — Septembre 2025 (sujets + corrections).
 * Usage: C:\xampp\php\php.exe scripts/seed_eo_septembre_2025.php
 */

$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
seed_eo_ensure_tables($pdo);

$dataFile = __DIR__ . '/../database/seeds/exp_orale/septembre_2025.json';
if (!is_file($dataFile)) {
    fwrite(STDERR, "Fichier donnees introuvable: $dataFile — lancez: python scripts/build_eo_septembre_2025_json.py\n");
    exit(1);
}
...
$slug = (string) ($meta['slug'] ?? 'eo-expression-orale-septembre-2025');
$title = (string) ($meta['title'] ?? 'Expression Orale Septembre 2025');
...
if ($existingId >= 0 && $existingId !== 0) {
    ...
    if ($nSub === 65) {
        echo "Epreuve deja presente (id=$existingId, slug=$slug, sujets=65). Rien a faire.\n";
        exit(0);
    }
...
try {
    ...
    $pdo->commit();
    echo "OK — Expression orale Septembre 2025 inseree (exam_id=$examId, parties=" . count($parts) . ", sujets=$subjectTotal).\n";
} catch (Throwable $e) {
    $pdo->rollBack();
