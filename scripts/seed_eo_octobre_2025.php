...
/**
 * Insere l'epreuve Expression orale — Octobre 2025 (sujets + corrections).
 * Usage: C:\xampp\php\php.exe scripts/seed_eo_octobre_2025.php
 */

$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
seed_eo_ensure_tables($pdo);

$dataFile = __DIR__ . '/../database/seeds/exp_orale/octobre_2025.json';
if (!is_file($dataFile)) {
    fwrite(STDERR, "Fichier donnees introuvable: $dataFile — lancez: python scripts/build_eo_octobre_2025_json.py\n");
    exit(1);
}
...
$slug = (string) ($meta['slug'] ?? 'eo-expression-orale-octobre-2025');
$title = (string) ($meta['title'] ?? 'Expression Orale Octobre 2025');
...
try {
    ...
    $pdo->commit();
    echo "OK — Expression orale Octobre 2025 inseree (exam_id=$examId, parties=" . count($parts) . ", sujets=$subjectTotal).\n";
} catch (Throwable $e) {
    $pdo->rollBack();
