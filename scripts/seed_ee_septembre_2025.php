...
/**
 * Insère l'épreuve Expression écrite — Septembre 2025.
 * Usage: C:\xampp\php\php.exe scripts\seed_ee_septembre_2025.php
 */
...
{
    $file = __DIR__ . '/../database/seeds/exp_ecrite/septembre_2025_data.php';
    if (!is_file($file)) {
...
$slug = 'ee-expression-ecrite-septembre-2025';
$title = 'Expression Écrite - Septembre 2025';
$subtitle = 'Sujets et corrections — Septembre 2025';
...
try {
    ...
    foreach ($combinations as $combo) {
        ...
        foreach (($combo['tasks'] ?? []) as $task) {
            ...
            $correction = ee_text_to_html((string) ($task['correction'] ?? ''));
            $insTask->execute([
                $comboId,
                $taskNum,
                $prompt,
    ...
    echo "Import réussi : exam_id=$examId, $comboCount combinaison(s).\n";
} catch (Throwable $e) {
    $pdo->rollBack();
