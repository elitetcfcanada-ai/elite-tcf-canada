...
/**
 * Insère l'épreuve Expression orale — Août 2025 (sujets + corrections).
 * Usage: C:\xampp\php\php.exe scripts/seed_eo_aout_2025.php
 */

$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
seed_eo_ensure_tables($pdo);

$dataFile = __DIR__ . '/../database/seeds/exp_orale/aout_2025.json';
if (!is_file($dataFile)) {
    fwrite(STDERR, "Fichier données introuvable: $dataFile — lancez: python scripts/build_eo_aout_2025_json.py\n");
    exit(1);
}
...
$slug = (string) ($meta['slug'] ?? 'eo-expression-orale-aout-2025');
$title = (string) ($meta['title'] ?? 'Expression Orale Août 2025');
$subtitle = (string) ($meta['subtitle'] ?? '');
$parts = $meta['parts'] ?? [];

if (!is_array($parts) || !$parts) {
    fwrite(STDERR, "Aucune partie dans les données.\n");
    exit(1);
}
...
if ($existingId >= 0 && $existingId !== 0) {
    ...
    if ($nSub === 40) {
        echo "Épreuve déjà présente (id=$existingId, slug=$slug, sujets=40). Rien à faire.\n";
        exit(0);
    }
    $pdo->prepare('DELETE FROM tcf_eo_exams WHERE id = ?')->execute([$existingId]);
    echo "Épreuve incomplète supprimée (id=$existingId, sujets=$nSub). Réimport…\n";
} elseif ($existingId === 0) {
...
try {
    ...
    $pdo->commit();
    echo "OK — Expression orale Août 2025 insérée (exam_id=$examId, parties=" . count($parts) . ", sujets=$subjectTotal).\n";
} catch (Throwable $e) {
    $pdo->rollBack();
