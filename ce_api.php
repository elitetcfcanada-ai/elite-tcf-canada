...
/** Consignes CE (une ligne éditoriale). */
...
function ce_seed_ce_consignes(PDO $pdo): void
{
    ...
    $n = (int) $pdo->query('SELECT COUNT(*) FROM tcf_ce_consignes')->fetchColumn();
    if ($n === 0) {
        $pdo->exec(
            "INSERT INTO tcf_ce_consignes (title, body, visibility, is_published) VALUES (
                'Consignes Compréhension Écrite',
...
try {
    switch ($action) {
        ...
        case 'save_exam': {
            ...
            try {
                if ($examId > 0) {
                    ...
                    // Envoyer notification à tous les utilisateurs connectés si l'épreuve est publiée
                    if ($isPublished) {
                        try {
                            ...
                            foreach ($users as $userId) {
                                tcf_notification_insert($pdo, (int) $userId, 'exam', 'Nouvelle épreuve publiée', "L'épreuve de compréhension écrite '$title' est maintenant disponible", 'comprehesion_ecrite.php');
                            }
                        } catch (Throwable $e) {
                            error_log('Erreur lors de l\'envoi des notifications épreuve: ' . $e->getMessage());
                        }
                    }
                }
        ...
        case 'get_consignes': {
            ...
            if ($canPremium) {
                $st = $pdo->query(
                    "SELECT id, title, body, visibility, is_published FROM tcf_ce_consignes WHERE is_published=1 ORDER BY id ASC LIMIT 1"
                );
            } else {
                $st = $pdo->query(
                    "SELECT id, title, body, visibility, is_published FROM tcf_ce_consignes WHERE is_published=1 AND visibility='gratuit' ORDER BY id ASC LIMIT 1"
                );
            }
            $row = $st->fetch(PDO::FETCH_ASSOC);
            $rows = $row ? [$row] : [];
        ...
        case 'get_consignes_bundle_admin': {
            ...
            ce_seed_ce_consignes($pdo);
            $row = $pdo->query('SELECT id, body, is_published FROM tcf_ce_consignes ORDER BY id ASC LIMIT 1')->fetch(PDO::FETCH_ASSOC);
            if (!$row) {
                ce_json(['success' => true, 'data' => ['body' => '', 'is_published' => 1]]);
            }
            ce_json([
                'success' => true,
                'data' => [
        ...
        case 'save_consignes_bundle': {
            ...
            if ($body === '') {
                ce_json(['success' => false, 'message' => 'Le texte des consignes est requis.'], 422);
            }
            ce_seed_ce_consignes($pdo);
            $id = (int) $pdo->query('SELECT MIN(id) FROM tcf_ce_consignes')->fetchColumn();
            if ($id <= 0) {
                ce_json(['success' => false, 'message' => 'Table consignes invalide.'], 500);
            }
            $pdo->prepare('UPDATE tcf_ce_consignes SET body=?, is_published=? WHERE id=?')->execute([$body, $isPublished, $id]);
