...
echo "<h1>Correction de la table tcf_ce_exams</h1>";

try {
    // D'abord supprimer les enregistrements avec id = 0
    $stmt = $pdo->query("SELECT COUNT(*) FROM tcf_ce_exams WHERE id = 0");
    $countZero = $stmt->fetchColumn();
    
    if ($countZero > 0) {
        echo "<p>Suppression des $countZero enregistrements avec id = 0...</p>";
        $pdo->exec("DELETE FROM tcf_ce_exams WHERE id = 0");
    ...
    $stmt = $pdo->query("SHOW KEYS FROM tcf_ce_exams WHERE Key_name = 'PRIMARY'");
    ...
    if ($hasPrimary) {
        ...
        $pdo->exec("ALTER TABLE tcf_ce_exams ADD PRIMARY KEY (id)");
    }
    
    // Vérifier l'auto_increment
    $stmt = $pdo->query("SHOW COLUMNS FROM tcf_ce_exams LIKE 'id'");
    ...
    if (strpos($colInfo['Extra'], 'auto_increment') !== false) {
        ...
        $pdo->exec("ALTER TABLE tcf_ce_exams MODIFY COLUMN id INT(10) UNSIGNED NOT NULL AUTO_INCREMENT");
    }
    
    
    // Réinitialiser l'auto_increment
    $pdo->exec("ALTER TABLE tcf_ce_exams AUTO_INCREMENT = 1");
    ...
    try {
        ...
        if ($countZeroQuestions > 0) {
            ...
            $pdo->exec("DELETE FROM tcf_ce_questions WHERE exam_id = 0");
        }
    } catch (PDOException $e) {
        echo "<p>Table tcf_ce_questions: " . $e->getMessage() . "</p>";
    }
    
    // tcf_ce_answers
    try {
        $stmt = $pdo->query("SELECT COUNT(*) FROM tcf_ce_answers WHERE question_id IN (SELECT id FROM tcf_ce_questions WHERE exam_id = 0)");
        $countZeroAnswers = $stmt->fetchColumn();
        if ($countZeroAnswers > 0) {
            echo "<p>Suppression de $countZeroAnswers réponses associées...</p>";
            $pdo->exec("DELETE FROM tcf_ce_answers WHERE question_id IN (SELECT id FROM tcf_ce_questions WHERE exam_id = 0)");
        }
    } catch (PDOException $e) {
        echo "<p>Table tcf_ce_answers: " . $e->getMessage() . "</p>";
    }
    
    echo "<h2>État final de tcf_ce_exams</h2>";
